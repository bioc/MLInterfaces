...
\section{Supporting the NOTEST type more broadly}

Consider the aim of using MLInterfaces to apply SVM to
a complete dataset without decomposition into test and training sets.
It is natural to consider using xvalSpec("NOTEST"), or supplying
the full vector of sample indices for the input data, but this
can cause problems, at least with svmI, as we have
...
function (obj, data, trainInd) 
{
    ...
    new("classifierOutput", testPredictions = factor(tepr[1:length(tepr)]), 
        trainPredictions = factor(trpr[1:length(trpr)]), testScores = attr(tepr, 
...
To proceed without generating problems, create a modified learnerSchema
interface.
<<domo>>=
svmI2 = svmI
svmI2@converter = function(obj, data, trainInd) {
    trData = data[trainInd, ]
    teData = trData # resubstitution
    tepr = predict(obj, teData, decision.values = TRUE, probability = TRUE)
    trpr = predict(obj, trData, decision.values = TRUE, probability = TRUE)
    new("classifierOutput", testPredictions = factor(tepr[1:length(tepr)]), 
        trainPredictions = factor(trpr[1:length(trpr)]), testScores = attr(tepr, 
...
You can now use svmI2 in place of svmI and avoid the errors
engendered by a vacuous index set for the test set, but a warning
will be thrown.
